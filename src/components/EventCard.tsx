...
import type { SDAEvent } from "@/lib/events-data";
...
const EventCard = ({ event }: { event: SDAEvent }) => {
      ...
      <div className="flex flex-1 flex-col gap-2 p-4">
        ...
        <div className="mt-auto flex flex-col gap-1.5 pt-2">
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            ...
            <span>{new Date(event.date).toLocaleDateString("en-KE", { dateStyle: "medium" })} • {event.time}</span>
          </div>
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
